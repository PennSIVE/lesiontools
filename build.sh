...
docker run --rm repronim/neurodocker:master generate docker \
    ...
    --run "apt-get update && apt-get install -y \
            ...
            --afni version=latest \
            --ants version=2.3.4 \
    | docker build -t lesionclusters_base -

docker build -t pennsive/lesionclusters:latest .
id=$(docker images --format="{{.Repository}}:{{.Tag}} {{.ID}}" | egrep "^pennsive/lesionclusters:latest " | cut -d' ' -f2)
version=$(cat version)
docker tag $id pennsive/lesionclusters:$version
docker push pennsive/lesionclusters:latest
docker push pennsive/lesionclusters:$version
