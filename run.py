...
parser.add_argument("--smooth", help="Smoothing value", nargs="?", default=1.2)
parser.add_argument(
    "--min-center-size",
    dest="minCenterSize",
    help="Minimum center size",
    nargs="?",
    default=10,
)
parser.add_argument("--cpus", help="Number of cores to use", nargs="?", default=1)
parser.add_argument(
    "--gmm", help="Whether to use gaussian mixture modeling", action="store_true"
)
parser.add_argument(
    "--bids-filter-file",
    dest="bids_filters",
...
if args.analysis_level == "participant":
    ...
    for subject_label in subjects_to_analyze:
        ...
        for i in range(len(sorted_t1s)):
            ...
            cmd = "/wrappers/mimosa.R --outdir %s --indir %s --flair %s --t1 %s --strip %s --thresh %s --register-to %s" % (
            ...
            cmd = "/wrappers/lesionclusters.R --outdir %s --probmap %s --binmap %s --smooth %s --minCenterSize %s" % (
                args.output_dir + outdir_suffix,
                os.path.join(args.output_dir + outdir_suffix, "probability_map.nii.gz"),
                os.path.join(args.output_dir + outdir_suffix, "binary_map.nii.gz"),
                args.smooth,
                args.minCenterSize,
            )
            if args.gmm:
    ...
    print("Only --analysis_level=participant is supported")
